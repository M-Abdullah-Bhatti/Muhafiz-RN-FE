...
const SolvedComplaints = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const getData = async () => {
        try {
          ...
          //   -------------------------
          const response = await GetAllData(
            `/incident/getAllIncidents?category=Theft&resolved=${resolved}`
          );

          if (response.success) {
            console.log("response: ", response);
            setError("");
            setComplaints(response?.data?.incidents);
            setResolvedComplaints(response?.data?.resolvedComplaintCount);
          } else {
    ...
    }, [selectedTab]) // Include currentPage in the dependency array
  );
              ...
              renderItem={({ item }) => (
                <View style={styles.complaintItem}>
                  ...
                  <Button
                    ...
                    type="clear"
                    onPress={() => {
                      navigation.navigate("ComplaintBoxInfo", {
                        complaint: item,
                      });
                    }}
