...
import { View, StyleSheet, TouchableOpacity, Text, Image, Alert, Linking, Platform } from 'react-native';
...
const MapWithButtons = () => {
  ...
  const sendLocationByEmail = async () => {
    ...
    const body = encodeURIComponent("Here is my current location: " + currentLocationUrl);
    const mailtoLink = `mailto:${email}?subject=${subject}&body=${body}`;
    
    // Attempt to open the mail link
    Linking.openURL(mailtoLink).catch(err => console.error('Error opening mail app', err));
  };
  
  
  // Use this function in a button onPress for example:
  <TouchableOpacity onPress={sendLocationByEmail} style={styles.sendEmailButton}>
  ...
  <Text style={styles.paragraph}>
    Lorem ipsum dolor sit amet. Sed nec sem id ex ultricies auctor vel non urna.
  </Text>
  {/* Display the currentLocationUrl */}
  {currentLocationUrl ? (
    <View style={styles.urlContainer}>
      <Text style={styles.urlText}>Your current location URL:</Text>
      <Text style={styles.url} onPress={() => Linking.openURL(currentLocationUrl)}>
        Open in Maps
      </Text>
    </View>
  ) : null}
